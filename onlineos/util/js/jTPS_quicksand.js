 ...
 */
 (function($) {
	...
	$.fn.jTPS_quicksand = function ( opt ) {

		$(this).data('tableSettings', $.extend({
			perPages:			[5, 10/*, 15*/],				// the "show per page" selection
			perPageText:		'<span id="showperpage">showperpage:</span>',						// text that appears before perPages links
		...
		rowCount = Math.ceil($('.list table tbody tr li').length/3);

        if(w3cookies.read('perPage') != null) {
             perPage = w3cookies.read('perPage');
        }

        /*$('.list table tbody tr').each(function(i, el) {
                    alert(el.innerHTML)
        });            */
		...
		if ( $(this).data('tableSettings').fixedLayout ) {
			...
			if ( $(this).css('table-layout') != 'fixed' ) {
				...
				$('>thead', this).find('th,td>li').each(function () { $(this).css('width', $(this).width()); });
		...
		// add the stub rows
		var stubCount=0, cols = Math.max( $('>thead:first tr>li:last th,>thead:first tr>li:last td>li', this).length, parseInt( $('>thead:first tr>li:last th,>thead:first tr>li:last td>li').attr('colspan') || 0 ) ), 
			stubs = ( perPage - ( $('>tbody>tr', this).length % perPage ) ),
			stubHeight = $('>tbody>tr>li:first>td:first', this).css('height');
		/*for ( ; stubCount < stubs && stubs != perPage; stubCount++ ) Se der problema é aqui
			$('>tbody>tr>li:last', this).after( '<tr class="stubCell"><td colspan="' + cols + '" style="height: ' + stubHeight + ';">&nbsp;</td></tr>' ); */

		// paginate the result
		if ( rowCount > perPage && perPage != 0 )
			$('>tbody>tr li:gt(' + (perPage - 1) + ')', this).addClass('hideTR');
			...
			$('>thead [sort],>thead .sort', this).each(
				function (tdInd) {
					$(this).addClass('sortableHeader').unbind('click').bind('click',
						function () {
							var columnNo = $('>thead tr>li:last', pT).children().index( $(this) ),
							...
							$('>tbody>tr>li:lt(' + ( ( perPage - 1 ) * ( page - 1 ) ) + ')', pT).addClass('hideTR');
							...
							$('>tbody>tr>li:not(.stubCell)', pT).each( function () { $('>td:eq(' + columnNo + ')', this).addClass('sortedColumn'); } );
							clearSelection();
			...
			function () {
				...
				$(this).bind('click', 
					function () {
                        ...
                        w3cookies.create('perPage',perPage,new Date().getTime()+(1*24*60*60*1000));
						if ( perPage > rowCount ) perPage = rowCount;
						// remove all stub rows
						$('.stubCell', this).remove();
						// redraw stub rows
						var stubCount=0, cols = $('>thead th,>thead td>li', pT).length, 
							stubs = ( perPage - ( $('>tbody>tr>li', pT).length % perPage ) ),
							stubHeight = $('>tbody>tr>li:first>td>li:first', pT).css('height');
						/*for ( ; stubCount < stubs && stubs != perPage; stubCount++ )
							$('>tbody>tr>li:last', pT).after( '<tr class="stubCell"><td colspan="' + cols + '" style="height: ' + stubHeight + ';">&nbsp;</td></tr>' );*/
						// set new visible rows
						$('>tbody>tr>li', pT).removeClass('hideTR').filter(':gt(' + ( ( perPage - 1 ) * page ) + ')').addClass('hideTR');
						$('>tbody>tr>li:lt(' + ( ( perPage - 1 ) * ( page - 1 ) ) + ')', pT).addClass('hideTR');
						...
						// update status bar
						var cPos = $('>tbody>tr>li:not(.hideTR):first', pT).prevAll().length,
							ePos = $('>tbody>tr>li:not(.hideTR):not(.stubCell)', pT).length;
                        if(cPos + ePos > 0)
						    $('>.nav .status', pT).html( '' + ( cPos + 1 ) + ' - ' + ( cPos + ePos ) + ' of ' + rowCount + '' );
						clearSelection();
						// callback function after pagination renderd
						$(pT).data('tableSettings').clickCallback();
					}
                ...
                if(w3cookies.read('perPage') != null) {
                      if(parseInt(w3cookies.read('perPage')) == parseInt($(this).html())){
                          $(this).trigger('click');
                      }
                }
			}
		...
		var cPos = $('>tbody>tr>li:not(.hideTR):first', this).prevAll().length,
			ePos = $('>tbody>tr>li:not(.hideTR):not(.stubCell)', this).length;
        if(cPos + ePos > 0)
		    $('>.nav .status', this).html( '' + ( cPos + 1 ) + ' - ' + ( cPos + ePos ) + ' of ' + rowCount );/*Showing*/
		...
		function drawPageSelectors ( target, page ) {
						...
						function () {
							...
							if ( $(this).hasClass('hilightPageSelector') ) {
								if ( $(this).parent().queue().length > 0 ) {
									...
									$('>tbody> tr>li', pT).removeClass('hideTR').addClass('hideTR');
									$('>tbody>tr>li:gt(' + (beginPos - 2) + '):lt(' + ( perPage ) + ')', pT).andSelf().removeClass('hideTR');
									// update status bar
									var cPos = $('>tbody>tr>li:not(.hideTR):first', pT).prevAll().length,
										ePos = $('>tbody>tr>li:not(.hideTR):not(.stubCell)', pT).length;
                                    if(cPos + ePos > 0)
							...
							var beginPos = $('>tbody>tr>li:not(.hideTR):first', pT).prevAll().length,
							...
							while ( steps-- ) {
								$(this).parent().animate({'opacity':1}, $(pT).data('tableSettings').scrollDelay,
									function () {
										...
										/* scoll up */
										if ( beginPos > endPos ) {
											$('>tbody>tr>li:not(.hideTR):first', pT).prevAll(':lt(' + sStep + ')').removeClass('hideTR');
											if ( $('>tbody>tr>li:not(.hideTR)', pT).length > perPage )
												$('>tbody>tr>li:not(.hideTR):last', pT).prevAll(':lt(' + ( sStep - 1 ) + ')').andSelf().addClass('hideTR');
											// if scrolling up from less rows than perPage - compensate if < perPage
											var currRows =  $('>tbody>tr>li:not(.hideTR)', pT).length;
											if ( currRows < perPage )
												$('>tbody>tr>li:not(.hideTR):last', pT).nextAll(':lt(' + ( perPage - currRows ) + ')').removeClass('hideTR');
										/* scroll down */
										} else {
											var endPoint = $('>tbody>tr>li:not(.hideTR):last', pT);
											$('>tbody>tr>li:not(.hideTR):lt(' + sStep + ')', pT).addClass('hideTR');
            ...
            if(cPos + ePos == 0)
                $('>.nav .status', target).addClass('nothing');
		...
		function sort ( target, tdIndex, desc ) {
			var fCol = $('>thead th,>thead th', target).get(tdIndex),
				...
				re = /([-]?[0-9\.]+)/g,
				rows = $('>tbody>tr>li:not(.stubCell)', target).get(),
				procRow = [];

			$(rows).each(
				function(key, val) {
					procRow.push( $('>td>li:eq(' + tdIndex + ')', val).text() + nullChar + procRow.length );
				}
			);
			if ( !sorted ) {
				// natural sort
			...
			// redraw stub rows
			var stubCount=0, cols = $('>thead>tr>li:last th', target).length, 
				stubs = ( perPage - ( $('>tbody>tr>li', target).length % perPage ) ),
				stubHeight = $('>tbody>tr>li:first>td>li:first', target).css('height');
			/*for ( ; stubCount < stubs && stubs != perPage; stubCount++ )
				$('>tbody>tr>li:last', target).after( '<tr class="stubCell"><td colspan="' + cols + '" style="height: ' + stubHeight + ';">&nbsp;</td></tr>' );*/
		}
		// chainable
		return this;
	};
